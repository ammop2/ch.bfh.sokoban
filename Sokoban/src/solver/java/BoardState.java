...
/**
 * Represents a single Sokoban solver.java.BoardState
 * @author Stephen Zhou
 * @uni szz2002
 *
 */
...
//							newPos.x, newPos.y));
	...
	/**
	 * Returns the new solver.java.BoardState after moving a certain direction
	 * @param direction the direction to move
	 * @return the new solver.java.BoardState
	 ...
	 */
	public static BoardState parseBoardInput(Map map) throws IOException {
		...
		for(int r = 0; r < height; r++)
		{
			...
			{
				char cr = '#';
				int i = map.getFields()[r * map.getXSize() + c];
				switch (i) {
					case 0:
						cr = ' ';
						break;
				...
				byte field = charToField.get(cr);
				boardPoints[r][c] = field;
				if ((field & PLAYER) == PLAYER)
					player = new Point(r, c);
				if ((field & GOAL) == GOAL)
					goals.add(new Point(r, c));
				if ((field & BOX) == BOX)
					boxes.add(new Point(r, c));
			}
		}
